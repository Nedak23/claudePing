...
class CommandParser:
    ...
    def extract_repo_context(self, message: str) -> Tuple[Optional[str], str]:
        """
        Extract repository name and cleaned prompt from message.

        Args:
            message: User's message

        Returns:
            Tuple of (repo_name or None, cleaned_prompt)
        """
        message = message.strip()

        # Try inline repo patterns
        for pattern in self.inline_regex:
            match = pattern.match(message)
            if match:
                return match.group(1), match.group(2).strip()

        # No repo context found
        return None, message

    def is_repo_command(self, message: str) -> bool:
        """
        Check if message is a repository management command.

        Args:
            message: User's message

        Returns:
            True if it's a repo management command
        """
        intent = self.parse(message)
        return intent.type in ['switch_repo', 'list_repos', 'repo_info', 'repo_status']

    def suggest_repo_names(self, partial: str, available_repos: List[str]) -> List[str]:
        """
        Suggest repository names based on partial match.

        Args:
            partial: Partial repository name
        ...
        # Exact matches first
        exact = [r for r in available_repos if r.lower() == partial_lower]

        # Starts with matches
        starts = [r for r in available_repos if r.lower().startswith(partial_lower) and r not in exact]

        # Contains matches
        contains = [r for r in available_repos if partial_lower in r.lower() and r not in exact and r not in starts]
